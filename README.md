...
O GPT interpreta decisões, conduz a narrativa, cria e revisa conteúdos e resolve interações usando regras e snapshots carregados. O backend evita perda de estado, valida resultados, persiste alterações e aplica consequências autoritativas sem precisar ser chamado para cada microetapa.
...
Entidades, itens e interações são materializados quando se tornam relevantes. Depois da materialização, o GPT utiliza apenas os recursos existentes no snapshot, sem inventar ações, equipamentos, consumíveis, dinheiro ou recompensas durante a resolução.
...
- [`docs/04-integracao-gpt-backend.md`](docs/04-integracao-gpt-backend.md): snapshots, sessões versionadas, resolução local, revisão e persistência final.
...
- [`docs/11-sistema-de-inventario-itens-drops-e-saque.md`](docs/11-sistema-de-inventario-itens-drops-e-saque.md): definições e instâncias de itens, pilhas, peso, propriedade, consumíveis, munições, drops, coleta e saque.

## Estado atual

Os documentos representam uma especificação em evolução. Fórmulas, tempos, multiplicadores, preços, pesos de perícias, limiares de qualidade, pilhas, carga, durabilidade e faixas devem ser versionados e simulados antes de versões `v1.0`.
...
- O GPT pode criar e revisar conteúdos por operações válidas, mas não altera retroativamente snapshots em andamento sem migração ou evento explícito.
